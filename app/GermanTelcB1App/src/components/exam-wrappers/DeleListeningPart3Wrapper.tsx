...
interface DeleListeningPart3WrapperProps {
  testId: string;
  onComplete: (score: number, answers: UserAnswer[]) => void;
}

const DeleListeningPart3Wrapper: React.FC<DeleListeningPart3WrapperProps> = ({ testId, onComplete }) => {
  ...
  useEffect(() => {
    const loadExam = async () => {
      try {
        setIsLoading(true);
        const data = await dataService.getDeleListeningPart3Content();
        setSectionDetails(data.section_details || {
          title: 'DELE Listening Part 3',
        ...
        console.error('Error loading DELE Listening Part 3 exam:', error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <View style={styles.container}>
      <DeleListeningUI exam={exam} sectionDetails={sectionDetails} part={3} onComplete={onComplete} />
    </View>
  );
};
...
export default DeleListeningPart3Wrapper;
