...
interface DeleListeningPart4WrapperProps {
  testId: string;
  onComplete: (score: number, answers: UserAnswer[]) => void;
}

const DeleListeningPart4Wrapper: React.FC<DeleListeningPart4WrapperProps> = ({ testId, onComplete }) => {
  ...
  useEffect(() => {
    const loadExam = async () => {
      try {
        ...
        console.error('Error loading DELE Listening Part 4 exam:', error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <View style={styles.container}>
      <DeleListeningUI exam={exam} sectionDetails={sectionDetails} part={4} onComplete={onComplete} />
    </View>
  );
};
...
export default DeleListeningPart4Wrapper;
