...
interface DeleListeningPart5WrapperProps {
  testId: string;
  onComplete: (score: number, answers: UserAnswer[]) => void;
}

const DeleListeningPart5Wrapper: React.FC<DeleListeningPart5WrapperProps> = ({ testId, onComplete }) => {
  ...
  useEffect(() => {
    const loadExam = async () => {
      try {
        ...
        console.error('Error loading DELE Listening Part 5 exam:', error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <View style={styles.container}>
      <DeleListeningUI exam={exam} sectionDetails={sectionDetails} part={5} onComplete={onComplete} />
    </View>
  );
};
...
export default DeleListeningPart5Wrapper;
