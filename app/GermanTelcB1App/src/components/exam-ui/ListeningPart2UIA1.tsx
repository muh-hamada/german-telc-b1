...
interface ListeningPart2UIA1Props {
  exam: Exam;
  sectionDetails: any;
...
const ListeningPart2UIA1: React.FC<ListeningPart2UIA1Props> = ({ exam, sectionDetails, onComplete, initialAnswers }) => {
  ...
  const handleAnswerSelect = (questionId: number, answer: boolean) => {
    setUserAnswers(prev => {
      ...
      if (existing) {
        return prev.map(a =>
          a.questionId === questionId ? { ...a, answer: answer ? 'true' : 'false', isCorrect: answer, timestamp: Date.now() } : a
        );
      }
      return [...prev, { questionId, answer: answer ? 'true' : 'false', isCorrect: answer, timestamp: Date.now() }];
    });
  ...
  const getUserAnswer = (questionId: number): boolean | null => {
  ...
  const handlePlayAudio = async () => {
    ...
    try {
      ...
      console.log('[ListeningPart2A1] Playing audio from:', audioPath);
  ...
  const handleSubmit = () => {
    ...
    if (unansweredQuestions.length > 0) {
      Alert.alert(
        ...
        t('listening.part2.incompleteMessage', { count: unansweredQuestions.length }),
        [{ text: 'OK' }]
      );
      return;
    }
    ...
    exam.questions.forEach(question => {
      ...
      const isCorrect = userAnswer?.answer === 'true' === question.is_correct;
      if (isCorrect) {
        correctCount++;
      }
      answers.push({
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.scrollContent}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.metaInfo}>
          ...
          <Text style={styles.metaText}>
            📝 {exam.questions.length} {t('listening.part2.tasks')}
          </Text>
        </View>
      </View>
          ...
          {exam.questions.map((question) => {
            ...
            return (
              <View key={question.id} style={styles.tableRow}>
                <View style={[styles.tableCell, styles.statementCell]}>
                  <Text style={styles.statementNumber}>{question.id}.</Text>
                  <Text style={styles.statementText}>{question.question}</Text>
                </View>
                <View style={[styles.tableCell, styles.answerCell]}>
                  <TouchableOpacity
                    style={[
...
export default ListeningPart2UIA1;
