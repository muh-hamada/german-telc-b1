...
interface ListeningPart2A2UIProps {
  exam: Exam;
  sectionDetails: any;
...
const ListeningPart2A2UI: React.FC<ListeningPart2A2UIProps> = ({ exam, sectionDetails, onComplete, initialAnswers }) => {
  ...
  const handlePlayAudio = async () => {
    ...
    try {
      ...
      console.log('[ListeningPart2A2] Playing audio from:', audioPath);
  ...
  const handleSubmit = () => {
    ...
    if (unansweredQuestions.length > 0) {
      Alert.alert(
        t('listening.part2.a2.incomplete'),
        t('listening.part2.a2.incompleteMessage', { count: unansweredQuestions.length }),
        [{ text: 'OK' }]
      );
      return;
    }
...
export default ListeningPart2A2UI;
