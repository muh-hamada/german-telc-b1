...
interface UserAnswer {
  statementId: number;
  selectedAnswer: boolean | null; // true = correct, false = wrong, null = not answered
}
...
const ListeningPart1Screen: React.FC = () => {
  ...
  const currentExam = exams[0]; // For now, use first exam
    ...
    const audioSound = new Sound(
      ...
      (error: any) => {
        if (error) {
          ...
          Alert.alert(
            'Audio Fehler',
            'Die Audiodatei konnte nicht geladen werden. Bitte überprüfen Sie die Datei.',
            [{ text: 'OK' }]
          );
          setIsPlaying(false);
          return;
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
        ...
        {/* Submit Button (optional, for future implementation) */}
