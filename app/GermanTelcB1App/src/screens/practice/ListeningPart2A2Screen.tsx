...
import ListeningPart2A2UI from '../../components/exam-ui/ListeningPart2A2UI';
import Icon from 'react-native-vector-icons/FontAwesome';
import { UserAnswer, ExamResult , ExamProgress } from '../../types/exam.types';
...
type ListeningPart2A2RouteProp = RouteProp<HomeStackParamList, 'ListeningPart2A2'>;

const ListeningPart2A2Screen: React.FC = () => {
  const navigation = useNavigation();
  const route = useRoute<ListeningPart2A2RouteProp>();
  ...
  const loadData = async () => {
    try {
      ...
      setListeningData(data);
        ...
        const progress = getExamProgress('listening-part2', String(examId));
        if (progress?.answers && progress.answers.length > 0) {
          setSavedProgress(progress);
          setShowResumeModal(true);
        }
    } catch (err) {
      console.error('Error loading listening part 2 A2 data:', err);
  ...
  return (
    <View style={styles.container}>
      ...
      <ResultsModal
        ...
        onClose={async () => {
          ...
          if (examResult?.score !== undefined && examResult?.maxScore !== undefined) {
            const wasAutoCompleted = await autoMarkCompletedIfEligible(
              'listening-part2', examId, examResult.score, examResult.maxScore
            );
            if (wasAutoCompleted) {
        ...
        examTitle={`Listening Part 2 - ${currentExam?.title || examId}`}
        result={examResult}
      />
      <ReportIssueModal
...
export default ListeningPart2A2Screen;
