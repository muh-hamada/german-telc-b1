...
import { ReadingPart1A1Exam, UserAnswer, ExamResult , ExamProgress } from '../../types/exam.types';
...
const ReadingPart1A1Screen: React.FC = () => {
  ...
  const [currentExam, setCurrentExam] = useState<ReadingPart1A1Exam | null>(null);
  ...
  const loadExam = async (id: number) => {
    try {
      setIsLoading(true);
      const exam = await dataService.getReadingPart1A1ExamById(id);
      if (exam) {
        setCurrentExam(exam);
  ...
  return (
    <View style={styles.container}>
      ...
      />
      <ReadingPart1A1UI key={uiKey} exam={currentExam} onComplete={handleComplete} initialAnswers={resumedAnswers} />
...
export default ReadingPart1A1Screen;
