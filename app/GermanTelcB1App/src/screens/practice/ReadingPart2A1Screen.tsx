...
import { ReadingPart2A1Exam, UserAnswer, ExamResult , ExamProgress } from '../../types/exam.types';
import ReadingPart2A1UI from '../../components/exam-ui/ReadingPart2A1UI';
import { HomeStackRouteProp } from '../../types/navigation.types';
import { AnalyticsEvents, logEvent } from '../../services/analytics.events';
import ResumeExamModal from '../../components/ResumeExamModal';

const ReadingPart2A1Screen: React.FC = () => {
  ...
  const loadExam = async (id: number) => {
    try {
      setIsLoading(true);
      const exam = await dataService.getReadingPart2A1ExamById(id);
      if (exam) {
        setCurrentExam(exam);
  ...
  return (
    <View style={styles.container}>
      ...
      />
      <ReadingPart2A1UI key={uiKey} exam={currentExam} onComplete={handleComplete} initialAnswers={resumedAnswers} />
...
export default ReadingPart2A1Screen;
