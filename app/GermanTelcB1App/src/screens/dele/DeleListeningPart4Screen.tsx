...
const DeleListeningPart4Screen: React.FC = () => {
  const { t } = useCustomTranslation();
  const route = useRoute<HomeStackRouteProp<'DeleListeningPart4'>>();
  ...
  const { isCompleted, toggleCompletion } = useExamCompletion('dele-listening', 4, examIdNum);
  ...
  const handleToggleCompletion = async () => {
    try {
      ...
      logEvent(AnalyticsEvents.PRACTICE_MARK_COMPLETED_TOGGLED, { section: 'dele-listening', part: 4, exam_id: examId, completed: newStatus });
    } catch (error: any) {
  ...
  const loadExam = async (id: string) => {
    try {
      setIsLoading(true);
      const exam = await dataService.getDeleListeningPart4ExamById(id);
      if (exam) {
        setCurrentExam(exam);
        setSectionDetails({
          title: exam.title || 'DELE Listening Part 4',
  ...
  const handleComplete = (score: number, answers: UserAnswer[]) => {
    ...
    updateExamProgress('dele-listening-part4', examIdNum, answers, score, totalQuestions);
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <ResultsModal
        ...
        examTitle={`DELE Listening Part 4 - ${currentExam.title}`}
        result={examResult}
      />
...
export default DeleListeningPart4Screen;
