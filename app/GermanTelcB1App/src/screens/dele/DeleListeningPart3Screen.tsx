...
const DeleListeningPart3Screen: React.FC = () => {
  const { t } = useCustomTranslation();
  const route = useRoute<HomeStackRouteProp<'DeleListeningPart3'>>();
  ...
  const { isCompleted, toggleCompletion } = useExamCompletion('dele-listening', 3, examIdNum);
  ...
  const handleToggleCompletion = async () => {
    try {
      ...
      logEvent(AnalyticsEvents.PRACTICE_MARK_COMPLETED_TOGGLED, { section: 'dele-listening', part: 3, exam_id: examId, completed: newStatus });
    } catch (error: any) {
  ...
  const loadExam = async (id: string) => {
    try {
      setIsLoading(true);
      const exam = await dataService.getDeleListeningPart3ExamById(id);
      if (exam) {
        setCurrentExam(exam);
        setSectionDetails({
          title: exam.title || 'DELE Listening Part 3',
  ...
  const handleComplete = (score: number, answers: UserAnswer[]) => {
    ...
    updateExamProgress('dele-listening-part3', examIdNum, answers, score, totalQuestions);
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <ResultsModal
        ...
        examTitle={`DELE Listening Part 3 - ${currentExam.title}`}
        result={examResult}
      />
...
export default DeleListeningPart3Screen;
