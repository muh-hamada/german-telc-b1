...
import DeleSpeakingPart2UI from '../../components/exam-ui/DeleSpeakingPart2UI';
import { HomeStackRouteProp } from '../../types/navigation.types';
import { AnalyticsEvents, logEvent } from '../../services/analytics.events';

const DeleSpeakingPart2Screen: React.FC = () => {
  const { t } = useCustomTranslation();
  const route = useRoute<HomeStackRouteProp<'DeleSpeakingPart2'>>();
  ...
  const { isCompleted, toggleCompletion } = useExamCompletion('dele-speaking', 2, topicId);
  ...
  const handleToggleCompletion = async () => {
    try {
      ...
      logEvent(AnalyticsEvents.PRACTICE_MARK_COMPLETED_TOGGLED, { section: 'dele-speaking', part: 2, exam_id: topicId, completed: newStatus });
    } catch (error: any) {
  ...
  const handleComplete = (score: number, answers: UserAnswer[]) => {
    ...
    updateExamProgress('dele-speaking-part2', topicId, answers, score, 1);
  };
  ...
  return (
    <View style={styles.container}>
      <DeleSpeakingPart2UI topic={currentTopic} onComplete={handleComplete} />
      ...
      <ResultsModal
        ...
        examTitle={`DELE Speaking Part 2 - ${currentTopic.title}`}
        result={examResult}
      />
...
export default DeleSpeakingPart2Screen;
