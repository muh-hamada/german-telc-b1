...
import { DeleReadingPart3Exam, UserAnswer, ExamResult } from '../../types/exam.types';
...
const DeleReadingPart3Screen: React.FC = () => {
  const { t } = useCustomTranslation();
  const route = useRoute<HomeStackRouteProp<'DeleReadingPart3'>>();
  ...
  const { isCompleted, toggleCompletion } = useExamCompletion('dele-reading', 3, examIdNum);
  
  const [currentExam, setCurrentExam] = useState<DeleReadingPart3Exam | null>(null);
  ...
  const handleToggleCompletion = async () => {
    try {
      ...
      logEvent(AnalyticsEvents.PRACTICE_MARK_COMPLETED_TOGGLED, { section: 'dele-reading', part: 3, exam_id: examId, completed: newStatus });
    } catch (error: any) {
  ...
  const loadExam = async (id: string) => {
    try {
      setIsLoading(true);
      const exam = await dataService.getDeleReadingPart3ExamById(id);
      if (exam) {
        setCurrentExam(exam);
  ...
  const handleComplete = (score: number, answers: UserAnswer[]) => {
    ...
    updateExamProgress('dele-reading-part3', examIdNum, answers, score, totalQuestions);
  };
  ...
  return (
    <View style={styles.container}>
      <DeleReadingPart3UI exam={currentExam} onComplete={handleComplete} />
      ...
      <ResultsModal
        ...
        examTitle={`DELE Reading Part 3 - ${currentExam.title}`}
        result={examResult}
      />
...
export default DeleReadingPart3Screen;
