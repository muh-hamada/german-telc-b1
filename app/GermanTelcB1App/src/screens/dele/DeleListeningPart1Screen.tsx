...
const DeleListeningPart1Screen: React.FC = () => {
  const { t } = useCustomTranslation();
  const route = useRoute<HomeStackRouteProp<'DeleListeningPart1'>>();
  ...
  const { isCompleted, toggleCompletion } = useExamCompletion('dele-listening', 1, examIdNum);
  ...
  const handleToggleCompletion = async () => {
    try {
      ...
      logEvent(AnalyticsEvents.PRACTICE_MARK_COMPLETED_TOGGLED, { section: 'dele-listening', part: 1, exam_id: examId, completed: newStatus });
    } catch (error: any) {
  ...
  const loadExam = async (id: string) => {
    try {
      setIsLoading(true);
      const exam = await dataService.getDeleListeningPart1ExamById(id);
      if (exam) {
        setCurrentExam(exam);
        // Create default section details for DELE Listening Part 1
        setSectionDetails({
          title: exam.title || 'DELE Listening Part 1',
  ...
  const handleComplete = (score: number, answers: UserAnswer[]) => {
    ...
    updateExamProgress('dele-listening-part1', examIdNum, answers, score, totalQuestions);
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <ResultsModal
        ...
        examTitle={`DELE Listening Part 1 - ${currentExam.title}`}
        result={examResult}
      />
...
export default DeleListeningPart1Screen;
