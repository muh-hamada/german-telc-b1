...
const DeleListeningPart5Screen: React.FC = () => {
  const { t } = useCustomTranslation();
  const route = useRoute<HomeStackRouteProp<'DeleListeningPart5'>>();
  ...
  const { isCompleted, toggleCompletion } = useExamCompletion('dele-listening', 5, examIdNum);
  ...
  const handleToggleCompletion = async () => {
    try {
      ...
      logEvent(AnalyticsEvents.PRACTICE_MARK_COMPLETED_TOGGLED, { section: 'dele-listening', part: 5, exam_id: examId, completed: newStatus });
    } catch (error: any) {
  ...
  const loadExam = async (id: string) => {
    try {
      setIsLoading(true);
      const exam = await dataService.getDeleListeningPart5ExamById(id);
      if (exam) {
        setCurrentExam(exam);
        setSectionDetails({
          title: exam.title || 'DELE Listening Part 5',
  ...
  const handleComplete = (score: number, answers: UserAnswer[]) => {
    ...
    updateExamProgress('dele-listening-part5', examIdNum, answers, score, totalQuestions);
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <ResultsModal
        ...
        examTitle={`DELE Listening Part 5 - ${currentExam.title}`}
        result={examResult}
      />
...
export default DeleListeningPart5Screen;
