...
import { DeleGrammarPart1Exam, UserAnswer, ExamResult } from '../../types/exam.types';
...
const DeleGrammarPart1Screen: React.FC = () => {
  const { t } = useCustomTranslation();
  const route = useRoute<HomeStackRouteProp<'DeleGrammarPart1'>>();
  ...
  const { isCompleted, toggleCompletion } = useExamCompletion('dele-grammar', 1, examIdNum);
  
  const [currentExam, setCurrentExam] = useState<DeleGrammarPart1Exam | null>(null);
  ...
  const handleToggleCompletion = async () => {
    try {
      ...
      logEvent(AnalyticsEvents.PRACTICE_MARK_COMPLETED_TOGGLED, { section: 'dele-grammar', part: 1, exam_id: examId, completed: newStatus });
    } catch (error: any) {
  ...
  const loadExam = async (id: string) => {
    try {
      setIsLoading(true);
      const exam = await dataService.getDeleGrammarPart1ExamById(id);
      if (exam) {
        setCurrentExam(exam);
  ...
  const handleComplete = (score: number, answers: UserAnswer[]) => {
    ...
    updateExamProgress('dele-grammar-part1', examIdNum, answers, score, totalQuestions);
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <ResultsModal
        ...
        examTitle={`DELE Grammar Part 1 - ${currentExam.title}`}
        result={examResult}
      />
...
export default DeleGrammarPart1Screen;
