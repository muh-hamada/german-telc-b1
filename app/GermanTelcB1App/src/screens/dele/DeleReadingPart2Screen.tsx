...
import { DeleReadingPart2Exam, UserAnswer, ExamResult } from '../../types/exam.types';
...
const DeleReadingPart2Screen: React.FC = () => {
  const { t } = useCustomTranslation();
  const route = useRoute<HomeStackRouteProp<'DeleReadingPart2'>>();
  ...
  const { isCompleted, toggleCompletion } = useExamCompletion('dele-reading', 2, examIdNum);
  
  const [currentExam, setCurrentExam] = useState<DeleReadingPart2Exam | null>(null);
  ...
  const handleToggleCompletion = async () => {
    try {
      ...
      logEvent(AnalyticsEvents.PRACTICE_MARK_COMPLETED_TOGGLED, { section: 'dele-reading', part: 2, exam_id: examId, completed: newStatus });
    } catch (error: any) {
  ...
  const loadExam = async (id: string) => {
    try {
      setIsLoading(true);
      const exam = await dataService.getDeleReadingPart2ExamById(id);
      if (exam) {
        setCurrentExam(exam);
  ...
  const handleComplete = (score: number, answers: UserAnswer[]) => {
    ...
    updateExamProgress('dele-reading-part2', examIdNum, answers, score, totalQuestions);
  };
  ...
  return (
    <View style={styles.container}>
      <DeleReadingPart2UI exam={currentExam} onComplete={handleComplete} />
      ...
      <ResultsModal
        ...
        examTitle={`DELE Reading Part 2 - ${currentExam.title}`}
        result={examResult}
      />
...
export default DeleReadingPart2Screen;
