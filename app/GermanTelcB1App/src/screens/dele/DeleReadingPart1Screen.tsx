...
const DeleReadingPart1Screen: React.FC = () => {
  ...
  const examIdNum = Number.parseInt(examId, 10) || 1; // Convert string ID to number for hooks
  
  const { isCompleted, toggleCompletion } = useExamCompletion('dele-reading', 1, examIdNum);
  
  const [currentExam, setCurrentExam] = useState<DeleReadingPart1Exam | null>(null);
  ...
  const handleToggleCompletion = async () => {
    try {
      ...
      logEvent(AnalyticsEvents.PRACTICE_MARK_COMPLETED_TOGGLED, { section: 'dele-reading', part: 1, exam_id: examId, completed: newStatus });
    } catch (error: any) {
  ...
  const loadExam = async (id: string) => {
    try {
      setIsLoading(true);
      const exam = await dataService.getDeleReadingPart1ExamById(id);
      if (exam) {
        setCurrentExam(exam);
  ...
  const handleComplete = (score: number, answers: UserAnswer[]) => {
    ...
    updateExamProgress('dele-reading-part1', examIdNum, answers, score, totalQuestions);
  };
  ...
  return (
    <View style={styles.container}>
      <DeleReadingPart1UI exam={currentExam} onComplete={handleComplete} />
      ...
      <ResultsModal
        ...
        examTitle={`DELE Reading Part 1 - ${currentExam.title}`}
        result={examResult}
      />
...
export default DeleReadingPart1Screen;
