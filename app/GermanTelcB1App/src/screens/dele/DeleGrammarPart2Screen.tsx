...
import { DeleGrammarPart2Exam, UserAnswer, ExamResult } from '../../types/exam.types';
...
const DeleGrammarPart2Screen: React.FC = () => {
  const { t } = useCustomTranslation();
  const route = useRoute<HomeStackRouteProp<'DeleGrammarPart2'>>();
  ...
  const { isCompleted, toggleCompletion } = useExamCompletion('dele-grammar', 2, examIdNum);
  
  const [currentExam, setCurrentExam] = useState<DeleGrammarPart2Exam | null>(null);
  ...
  const handleToggleCompletion = async () => {
    try {
      ...
      logEvent(AnalyticsEvents.PRACTICE_MARK_COMPLETED_TOGGLED, { section: 'dele-grammar', part: 2, exam_id: examId, completed: newStatus });
    } catch (error: any) {
  ...
  const loadExam = async (id: string) => {
    try {
      setIsLoading(true);
      const exam = await dataService.getDeleGrammarPart2ExamById(id);
      if (exam) {
        setCurrentExam(exam);
  ...
  const handleComplete = (score: number, answers: UserAnswer[]) => {
    ...
    updateExamProgress('dele-grammar-part2', examIdNum, answers, score, totalQuestions);
  };
  ...
  return (
    <View style={styles.container}>
      <DeleGrammarPart2UI exam={currentExam} onComplete={handleComplete} />
      ...
      <ResultsModal
        ...
        examTitle={`DELE Grammar Part 2 - ${currentExam.title}`}
        result={examResult}
      />
...
export default DeleGrammarPart2Screen;
