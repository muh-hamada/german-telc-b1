...
const DeleWritingPart2Screen: React.FC = () => {
  const { t } = useCustomTranslation();
  const route = useRoute<HomeStackRouteProp<'DeleWritingPart2'>>();
  ...
  const { isCompleted, toggleCompletion } = useExamCompletion('dele-writing', 2, examIdNum);
  ...
  const loadExam = async () => {
    try {
      setIsLoading(true);
      const exam = await dataService.getDeleWritingPart2ExamById(examId);
  ...
  const handleComplete = (score: number) => {
    ...
    updateExamProgress('dele-writing-part2', examIdNum, [], score, maxScore);
  };
  ...
  return (
    <View style={styles.container}>
      <DeleWritingPart2UI exam={currentExam} onComplete={handleComplete} />
...
export default DeleWritingPart2Screen;
