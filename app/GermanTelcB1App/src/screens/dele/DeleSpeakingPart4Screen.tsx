...
import DeleSpeakingPart4UI from '../../components/exam-ui/DeleSpeakingPart4UI';
import { HomeStackRouteProp } from '../../types/navigation.types';
import { AnalyticsEvents, logEvent } from '../../services/analytics.events';

const DeleSpeakingPart4Screen: React.FC = () => {
  const { t } = useCustomTranslation();
  const route = useRoute<HomeStackRouteProp<'DeleSpeakingPart4'>>();
  ...
  const { isCompleted, toggleCompletion } = useExamCompletion('dele-speaking', 4, topicId);
  ...
  const handleToggleCompletion = async () => {
    try {
      ...
      logEvent(AnalyticsEvents.PRACTICE_MARK_COMPLETED_TOGGLED, { section: 'dele-speaking', part: 4, exam_id: topicId, completed: newStatus });
    } catch (error: any) {
  ...
  const handleComplete = (score: number, answers: UserAnswer[]) => {
    ...
    updateExamProgress('dele-speaking-part4', topicId, answers, score, 1);
  };
  ...
  return (
    <View style={styles.container}>
      <DeleSpeakingPart4UI topic={currentTopic} onComplete={handleComplete} />
      ...
      <ResultsModal
        ...
        examTitle={`DELE Speaking Part 4 - ${currentTopic.title}`}
        result={examResult}
      />
...
export default DeleSpeakingPart4Screen;
