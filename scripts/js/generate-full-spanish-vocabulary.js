...
// Configuration
const INPUT_FILE = 'spanish-b1-vocabulary-raw.json';
const OUTPUT_FILE = 'spanish-b1-vocabulary-complete.json';
const PROGRESS_FILE = 'spanish-b1-translation-progress.json';
...
async function translateWithOpenAI(spanishWord) {
  const prompt = `Analyze and translate the following Spanish vocabulary word.

Spanish word: "${spanishWord}"

Please provide:
1. The word type (noun, verb, adjective, adverb, pronoun, preposition, conjunction, etc.)
2. If it's a noun, provide the appropriate article (el, la, los, las)
3. Generate 3 natural example sentences in Spanish using this word
4. Translate the word and all example sentences to: English (en), German (de), French (fr), Russian (ru), Arabic (ar)
5. The "word" field should contain the Spanish word WITHOUT the article if it's a noun. If the provided spanish word includes an article, remove it in the "word" field.

Return the response in the following JSON format:
{
  "type": "noun|verb|adjective|adverb|pronoun|preposition|conjunction|etc",
  "article": "el|la|los|las|" (empty string if not a noun),
  "word": "Spanish word WITHOUT article if it's a noun",
  "wordTranslations": {
  ...
  "exampleSentences": [
    {
      "text": "Spanish example sentence 1",
      "translations": {
        "en": "English translation",
        "de": "German translation",
        "fr": "French translation",
    ...
    {
      "text": "Spanish example sentence 2",
      "translations": {
        "en": "English translation",
        "de": "German translation",
        "fr": "French translation",
    ...
    {
      "text": "Spanish example sentence 3",
      "translations": {
        "en": "English translation",
        "de": "German translation",
        "fr": "French translation",
...
- Example sentences should be at B1 level (intermediate)
- Maintain the same meaning and context across all translations
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a professional translator and linguist specializing in Spanish language. You provide accurate translations in multiple languages, identify word types, and create natural example sentences at B1 (intermediate) level. Always respond with valid JSON only.'
          },
          {
            role: 'user',
            content: prompt
...
async function processEntry(entry) {
  ...
  try {
    const result = await translateWithOpenAI(entry.word);
    
    const processedEntry = {
      word: result.word || entry.word,
      article: result.article || "",
      type: result.type,
      translations: result.wordTranslations,
...
async function main() {
  ...
  console.log('🚀 Spanish B1 Vocabulary Generation Script\n');
