...
// Configuration
const INPUT_FILE = 'english-b2-words.json';
const OUTPUT_FILE = 'english-b2-vocabulary-complete.json';
const PROGRESS_FILE = 'english-b2-translation-progress.json';
...
async function translateWithOpenAI(englishWord, englishSentences) {
  const prompt = `Translate the following English vocabulary entry to multiple languages and determine its grammatical type.
...
Please provide translations in the following format (JSON):
{
  ...
  "sentenceTranslations": [
    {
      "de": "German translation of sentence 1",
...
- For "type", use standard grammatical terms (noun, verb, adjective, etc.)
...
async function processEntry(entry) {
  ...
  const englishSentences = entry.exampleSentences.map(s => s.text);
  
  try {
    const translations = await translateWithOpenAI(entry.word, englishSentences);
    
    const processedEntry = {
      word: entry.word,
      type: translations.type || entry.type,
...
async function main() {
  ...
  console.log('🚀 B2 Vocabulary Translation Script\n');
